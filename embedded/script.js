...
if (placement_query != undefined && placement_query != null) {
	overlay_offsetY = placement_query.offsetTop;
}
...
function activate() {
	...
	function load_more_results(append, callback) {
		...
		searcher.yield_template(function (template, state) {
			...
			storage.put("filters", state.filters && state.filters.length ? state.filters : undefined);
			storage.put("sorting", state.sorting && state.sorting.length ? state.sorting : undefined);
...
function add_skippable_button(element) {
	if (!element) return;
	const skipButton = document.querySelector(".hr-overlay-search .skip-to-product");
